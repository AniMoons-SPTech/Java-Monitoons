        ...
        for (int i = 0; i < listaProcessos.size(); i++){
            ...
            for (int j = 0; j < listaJanelas.size(); j++){
                ...
                if (PIDJanela == PIDProcesso){
                    ...
                    aplicativo2 = new Aplicativo2(PIDJanela, janela.getTitulo(), janela.getComando(), df.format(processo.getUsoCpu()), df.format(processo.getUsoMemoria()));
...
//                        con.update("INSERT INTO appsAbertos (pid, fkComputadorApp,nomeAplicativo, comando, usoCPU, usoDisco, dtHora) VALUES (%d, %d, '%s', '%s', %s, %s, now() )".formatted(aplicativo2.getPID(), janela.getTitulo(), janela.getComando(), df.format(processo.getUsoCpu()), df.format(processo.getUsoMemoria())));
//                    }
//                }
//            }
//        }
