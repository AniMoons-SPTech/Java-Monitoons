...
public class Aplicativo {
    ...
    public void capturarAplicativos(Integer idComputador) {
        ...
        for (int i = 0; i < listaProcessos.size(); i++) {
            ...
            for (int j = 0; j < listaJanelas.size(); j++) {
                ...
                if (PIDJanela == PIDProcesso) {
                    ...
                    if ((janela.getTitulo() != "")) {
                        con.update("INSERT INTO appsAbertos (pid, fkComputadorApp,nomeAplicativo, comando, usoCPU, usoDisco, dtHora) VALUES (%d, %d, '%s', '%s', %s, %s, now() )".formatted(aplicativo2.getPID(), idComputador, janela.getTitulo(), janela.getComando(), df.format(processo.getUsoCpu()), df.format(processo.getUsoMemoria())));
                    }
                }
            }
        }
